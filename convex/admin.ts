...
export const getAdminForLogin = query({
  args: { email: v.string() },
  returns: v.union(v.null(), v.object({ email: v.string(), firstName: v.string(), lastName: v.string(), password: v.string() })),
...
export const getProfile = query({
  ...
  returns: v.union(v.null(), v.object({ email: v.string(), firstName: v.string(), lastName: v.string(), createdAt: v.number() })),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const admin = await getAdminByEmail(ctx, args.adminEmail)
    if (!admin) return null
    return { email: admin.email, firstName: admin.firstName, lastName: admin.lastName, createdAt: admin.createdAt }
  },
})

export const updateProfile = mutation({
  args: { adminEmail: v.string(), firstName: v.string(), lastName: v.string() },
  returns: v.object({ email: v.string(), firstName: v.string(), lastName: v.string() }),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const admin = await getAdminByEmail(ctx, args.adminEmail)
...
export const addAdmin = mutation({
  args: { adminEmail: v.string(), email: v.string(), firstName: v.string(), lastName: v.string(), password: v.string() },
  returns: v.object({ email: v.string(), firstName: v.string(), lastName: v.string() }),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
...
export const removeAdmin = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (targetEmail === requesterEmail) throw new Error('You cannot remove your own admin access.')

    const target = await getAdminByEmail(ctx, targetEmail)
    if (!target) throw new Error('Admin not found.')

    const allAdmins = await ctx.db.query('admins').collect()
    if (allAdmins.length <= 1) throw new Error('At least one admin must remain.')
...
export const updateAdmin = mutation({
  args: { adminEmail: v.string(), targetEmail: v.string(), firstName: v.string(), lastName: v.string() },
  returns: v.object({ email: v.string(), firstName: v.string(), lastName: v.string() }),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const targetEmail = normalizeEmail(args.targetEmail)
...
export const listAdmins = query({
  args: { adminEmail: v.string() },
  returns: v.array(v.object({ email: v.string(), firstName: v.string(), lastName: v.string(), createdAt: v.number() })),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const admins = await ctx.db.query('admins').collect()
    return admins.sort((a, b) => b.createdAt - a.createdAt).map((admin) => ({
      email: admin.email, firstName: admin.firstName, lastName: admin.lastName, createdAt: admin.createdAt,
    }))
  },
})
...
export const getAdminsForNotifications = query({
  args: {},
  returns: v.array(v.object({ email: v.string(), firstName: v.string(), lastName: v.string() })),
  handler: async (ctx) => {
    const admins = await ctx.db.query('admins').collect()
    return admins.map((admin) => ({ email: admin.email, firstName: admin.firstName, lastName: admin.lastName }))
  },
})
...
export const getStats = query({
  args: { adminEmail: v.string() },
  returns: v.object({
    totalUsers: v.number(), totalReports: v.number(), pendingReports: v.number(),
    activeDispatchReports: v.number(), completedReports: v.number(),
    wildlifeReports: v.number(), domesticReports: v.number(),
  }),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    ...
    return {
      ...
      pendingReports: reports.filter((r) => normalizeReportStatus(r.status) === 'pending').length,
      activeDispatchReports: reports.filter((r) => ['accepted', 'en_route'].includes(normalizeReportStatus(r.status))).length,
      completedReports: reports.filter((r) => ['rescue_success', 'rescue_failed'].includes(normalizeReportStatus(r.status))).length,
...
const reportAnalyticsSummaryValidator = v.object({
  totalUsers: v.number(), totalReports: v.number(), pendingReports: v.number(),
  activeDispatchReports: v.number(), completedReports: v.number(), wildlifeReports: v.number(),
  domesticReports: v.number(), totalRescuers: v.number(), unassignedPending: v.number(),
  reportsWithPhotos: v.number(), reportsWithGps: v.number(),
})

export const getReportAnalytics = query({
  args: { adminEmail: v.string(), days: analyticsDaysValidator },
  returns: v.object({
    summary: reportAnalyticsSummaryValidator,
    reportsOverTime: v.array(v.object({ date: v.string(), total: v.number(), wildlife: v.number(), domestic: v.number() })),
    byStatus: v.array(v.object({ status: v.union(v.literal('pending'), v.literal('accepted'), v.literal('en_route'), v.literal('rescue_success'), v.literal('rescue_failed'), v.literal('published'), v.literal('rejected')), label: v.string(), count: v.number() })),
    byCategory: v.array(v.object({ category: v.union(v.literal('wildlife'), v.literal('domestic')), label: v.string(), count: v.number() })),
    domesticByType: v.array(v.object({ type: v.string(), label: v.string(), count: v.number() })),
    wildlifeTopSpecies: v.array(v.object({ name: v.string(), count: v.number() })),
    wildlifeByCondition: v.array(v.object({ condition: v.string(), label: v.string(), count: v.number() })),
    rescuerWorkload: v.array(v.object({ email: v.string(), name: v.string(), active: v.number(), completed: v.number(), total: v.number() })),
    outcomes: v.object({ success: v.number(), failed: v.number() }),
  }),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
...
export const listReports = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [reports, users, rescuers] = await Promise.all([
      ctx.db.query('reports').collect(), ctx.db.query('users').collect(), ctx.db.query('rescuers').collect(),
    ])

    const usersByEmail = new Map(users.map((row) => [normalizeEmail(row.email), row]))
    const rescuersByEmail = new Map(rescuers.map((row) => [normalizeEmail(row.email), row]))
    ...
    return Promise.all(
      sorted.map(async (report) => {
        ...
        const assigned = report.assignedRescuerEmail ? rescuersByEmail.get(normalizeEmail(report.assignedRescuerEmail)) : undefined
        ...
        return {
          ...
          assignedRescuerName: assigned ? `${assigned.firstName} ${assigned.lastName}`.trim() : undefined,
        }
      }),
    )
  },
...
async function assignRescuerToReportHandler(ctx: MutationCtx, args: { reportId: Id<'reports'>; rescuerEmail: string; adminEmail?: string }) {
  ...
  if (isTerminalStatus(status)) throw new Error('Completed reports cannot be assigned to a rescuer.')
  if (status !== 'pending' && status !== 'accepted' && status !== 'en_route') throw new Error('This report cannot be assigned in its current status.')
  ...
  await ctx.db.patch(args.reportId, { ...(status === 'pending' ? { status: 'accepted' as const } : {}), assignedRescuerEmail: rescuerEmail })

  // NOTIFY RESCUER
  await ctx.scheduler.runAfter(0, internal.notifications.notifyRescuer, {
...
export const assignRescuerToReport = mutation({
  args: { adminEmail: v.string(), reportId: v.id('reports'), rescuerEmail: v.string() },
  returns: v.null(),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
...
export const deleteReport = mutation({
  args: { adminEmail: v.string(), reportId: v.id('reports') },
  returns: v.null(),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const doc = await ctx.db.get('reports', args.reportId)
...
export const acceptReport = mutation({
  args: { adminEmail: v.string(), reportId: v.id('reports') },
  returns: v.null(),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    ...
    if (!doc) throw new Error('Report not found.')
    const status = normalizeReportStatus(doc.status)
    if (status !== 'pending') throw new Error('Only pending reports can be accepted.')

    await ctx.db.patch(args.reportId, { status: 'accepted' })

    await writeAuditLog(ctx, {
      action: 'admin.report.accept',
      actorEmail: args.adminEmail,
      actorRole: 'admin',
      targetType: 'report',
      targetId: args.reportId,
      details: JSON.stringify({ animalName: doc.animalName }),
    })
...
export const rejectReport = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!doc) throw new Error('Report not found.')
    const status = normalizeReportStatus(doc.status)
    if (status !== 'pending') throw new Error('Only pending reports can be rejected.')

    await ctx.db.patch(args.reportId, { status: 'rejected' })

    await writeAuditLog(ctx, {
      action: 'admin.report.reject',
      actorEmail: args.adminEmail,
      actorRole: 'admin',
      targetType: 'report',
      targetId: args.reportId,
...
export const updateUser = mutation({
  args: { adminEmail: v.string(), userId: v.id('users'), firstName: v.string(), lastName: v.string() },
  returns: v.null(),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    ...
    await ctx.db.patch(args.userId, { firstName: args.firstName.trim(), lastName: args.lastName.trim() })
    return null
  },
})

export const deleteUser = mutation({
  args: { adminEmail: v.string(), userId: v.id('users') },
  returns: v.null(),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const target = await ctx.db.get('users', args.userId)
    ...
    if (targetIsAdmin) throw new Error('Admin accounts cannot be deleted from the users table.')
    if (normalizeEmail(target.email) === normalizeEmail(args.adminEmail)) throw new Error('You cannot delete your own account.')

    const reports = await ctx.db.query('reports').withIndex('by_user_email', (q) => q.eq('userEmail', target.email)).collect()
...
export const ensureAdminAccount = mutation({
  args: { adminEmail: v.string() },
  returns: v.object({ email: v.string(), firstName: v.string(), lastName: v.string(), role: userRoleValidator }),
  handler: async (ctx, args) => {
    await seedDefaultAdmin(ctx)
    const admin = await getAdminByEmail(ctx, args.adminEmail)
    ...
    if (existing) {
      await ctx.db.patch(existing._id, { role: 'admin', firstName: admin.firstName, lastName: admin.lastName })
      return { email, firstName: admin.firstName, lastName: admin.lastName, role: 'admin' as const }
    }

    await ctx.db.insert('users', { email, firstName: admin.firstName, lastName: admin.lastName, role: 'admin', createdAt: Date.now() })
    return { email, firstName: admin.firstName, lastName: admin.lastName, role: 'admin' as const }
  },
})

export const changeAdminPassword = mutation({
  args: { adminEmail: v.string(), targetEmail: v.string(), currentPassword: v.string(), newPassword: v.string() },
  returns: v.null(),
  handler: async (ctx, args) => {
    await assertAdmin(ctx, args.adminEmail)
    const targetEmail = normalizeEmail(args.targetEmail)
    ...
    if (storedPassword !== args.currentPassword) throw new Error('Incorrect current password.')
...
export const resetAdminPasswordWithOtp = mutation({
  args: { adminEmail: v.string(), targetEmail: v.string(), otpCode: v.string(), newPassword: v.string() },
  returns: v.null(),
  handler: async (ctx, args) => {
    const targetEmail = normalizeEmail(args.targetEmail)
    const target = await getAdminByEmail(ctx, targetEmail)
