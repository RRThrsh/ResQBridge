...
export default function InfoPopover({ children, className = '', position = 'bottom' }) {
  ...
  return (
    <span ref={ref} className={`relative inline-flex align-middle ${className}`}>
      ...
      {open && (
        <div className={`absolute left-0 z-[1000] w-72 rounded-lg bg-gray-900 px-3 py-2 text-xs text-white shadow-lg ${position === 'top' ? 'bottom-full mb-1' : 'top-full mt-1'}`}>{children}</div>
      )}
    </span>
  )
}
