...
const EVENT_CONFIG = {
  login:          { color: 'text-green-700', bg: 'bg-green-100', label: 'LOGIN', icon: '→' },
  login_attempt:  { color: 'text-yellow-700', bg: 'bg-yellow-100', label: 'ATTEMPT', icon: '?' },
  logout:         { color: 'text-red-700', bg: 'bg-red-100', label: 'LOGOUT', icon: '←' },
  register:       { color: 'text-emerald-700', bg: 'bg-emerald-100', label: 'REGISTER', icon: '+' },
  guest:          { color: 'text-purple-700', bg: 'bg-purple-100', label: 'GUEST', icon: '●' },
  password_reset: { color: 'text-orange-700', bg: 'bg-orange-100', label: 'RESET', icon: '🔑' },
  role_change:    { color: 'text-blue-700', bg: 'bg-blue-100', label: 'ROLE', icon: '⚡' },
  config_update:  { color: 'text-cyan-700', bg: 'bg-cyan-100', label: 'CONFIG', icon: '⚙' },
  landing_update: { color: 'text-pink-700', bg: 'bg-pink-100', label: 'LANDING', icon: '✎' },
}
...
function UserCell({ log, meta }) {
  if (log.userId) {
    return (
      <span className="truncate font-medium text-gray-900" title={`UUID: ${log.userId}`}>
...
function MetaPreview({ meta }) {
  ...
  return <span className="truncate text-gray-500">{String(text).slice(0, 80)}</span>
}
  ...
  return (
    <>
      <div
        ...
        className="flex cursor-pointer items-center gap-3 border-b border-gray-100 px-4 py-2 font-mono text-xs leading-5 transition-colors hover:bg-gray-50"
      >
        ...
        <svg className={`h-3 w-3 shrink-0 text-gray-400 transition-transform ${expanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </div>
      {expanded && (
        <div className="border-b border-gray-100 bg-gray-50 px-4 py-3 font-mono text-[11px] leading-relaxed text-gray-600">
          <div className="grid grid-cols-2 gap-x-8 gap-y-1.5">
            <div><span className="text-gray-500">Event Type:</span> {log.eventType}</div>
            <div><span className="text-gray-500">IP Address:</span> {ip}</div>
            <div><span className="text-gray-500">User ID:</span> {log.userId || 'N/A'}</div>
            <div><span className="text-gray-500">User Agent:</span> {(log.userAgent || 'N/A').slice(0, 80)}</div>
            <div><span className="text-gray-500">Section:</span> {log.section || 'N/A'}</div>
            <div><span className="text-gray-500">Duration:</span> {formatDuration(log.sessionDuration)}</div>
            <div><span className="text-gray-500">Created:</span> {formatDate(log.createdAt || log._creationTime)}</div>
            <div><span className="text-gray-500">Log ID:</span> {log._id}</div>
          </div>
          {meta && (
            <div className="mt-2">
              <div className="text-gray-500 mb-1">Metadata:</div>
              <pre className="overflow-x-auto rounded bg-white border border-gray-200 p-2 text-[11px] text-gray-700">{JSON.stringify(meta, null, 2)}</pre>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center gap-3 border-b border-gray-200 bg-gray-50 px-4 py-2 font-mono text-xs font-semibold uppercase tracking-wider text-gray-500">
        ...
        <div className="max-h-[65dvh] overflow-y-auto">
          {loading ? (
            <div className="flex items-center justify-center py-20 text-sm text-gray-500 font-mono">
