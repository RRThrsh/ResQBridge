  ...
  return (
    <div className="rounded-2xl border border-dashed border-border bg-card/40 px-6 py-16 text-center">
      <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 border border-primary/20">
        {tab === 'pending' ? <Clock className="h-6 w-6 text-primary" /> : <CheckCircle2 className="h-6 w-6 text-primary" />}
      </div>
      ...
      <p className="mx-auto mt-2 max-w-xs text-sm text-muted-foreground">
        {tab === 'pending' ? 'When a user submits a domestic report, it will appear here for your review.' : 'Approved domestic reports will show up here and on the public feed.'}
      </p>
    </div>
  )
}
...
export function DomesticReportsPage() {
  ...
  // FIX: We bypass the `rescuerReportToStored` helper here so the Card gets pristine Storage IDs
        ...
        <>
          ...
          <div className="mb-5 flex items-center justify-between gap-3">
            <div className="inline-flex rounded-xl border border-border bg-muted/40 p-1">
              {(
                [['pending', 'Needs Review'], ['published', 'Published']] as const
              ).map(([value, label]) => (
                <button
                ...
                >
                  {label}
                  <span className="ml-1.5 tabular-nums text-muted-foreground">({value === 'pending' ? pending.length : published.length})</span>
                </button>
              ))}
            </div>
          </div>
