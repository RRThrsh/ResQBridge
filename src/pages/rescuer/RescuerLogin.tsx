...
export function RescuerLogin() {
  ...
  async function sendCode(e: React.FormEvent) {
    ...
    try {
      ...
      await sendRescuerOtp(normalizedEmail)
      setEmail(normalizedEmail)
      sessionStorage.setItem(RESCUER_OTP_EMAIL_KEY, normalizedEmail)
      setStep('otp')
  ...
  async function verifyCode(e: React.FormEvent) {
    ...
    const otpEmail =
      ...
      sessionStorage.getItem(RESCUER_OTP_EMAIL_KEY)?.trim().toLowerCase() ||
      ''
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-background p-4">
      ...
      <Card className="w-full max-w-md border-border">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Rescuer sign-in only. Enter your authorized email to receive a verification code.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {step === 'email' ? (
            <form onSubmit={sendCode} className="space-y-4">
              <div>
                <label className="mb-1 block text-xs text-muted-foreground">Rescuer email</label>
                <Input
                  type="email"
                  value={email}
