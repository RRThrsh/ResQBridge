    ...
    <div className="min-h-screen pt-32 pb-20 flex items-center justify-center">
      <div className="text-center">
        <p className="text-lg font-semibold text-foreground mb-2">
          {t('reportSuccess.notFound')}
        </p>

        <p className="text-sm text-muted-foreground mb-6">
          {t('reportSuccess.notFoundDesc')}
        </p>

        <Link to="/">
          <Button>
            {t('reportSuccess.returnHome')}
          </Button>
        </Link>
      </div>
    </div>
    ...
    <div className="min-h-screen pt-32 pb-20 flex flex-col items-center justify-center">
      <div className="mx-auto max-w-md px-4 sm:px-6 w-full text-center">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed mb-8">
          {t('reportSuccess.desc')}
        </p>

        <div className="bg-card border border-border rounded-2xl p-5 mb-10 text-left">
          <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-3">{t('reportSuccess.submissionDetails')}</p>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground shrink-0">{t('reportSuccess.trackingId')}</span>
              <span className="font-mono text-foreground text-right">{latestReport.id.slice(-8).toUpperCase()}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground shrink-0">{t('reportSuccess.type')}</span>
              <span className="text-foreground capitalize text-right">{latestReport.type.replace(/-/g, ' ')}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground shrink-0">{t('reportSuccess.submitted')}</span>
              <span className="text-foreground text-right">
                {formatDateTime(latestReport.createdAt)}
              </span>
            </div>
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground shrink-0">{t('reportSuccess.status')}</span>
              <span className="text-amber-500 font-medium flex items-center gap-1.5 justify-end">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
                {t('reportSuccess.underReview')}
              </span>
            </div>
          </div>
        </div>
