...
import { AdminAddRescuerDialog } from '@/components/admin/AdminAddRescuerDialog'
import {
  AdminRescuerDialog,
...
export function AdminRescuersPage() {
  const { admin } = useAdminAuth()
  const removeRescuer = useMutation(api.rescuers.removeRescuer)
  const rescuers = useQuery(
    api.rescuers.listRescuers,
    admin ? { adminEmail: normalizeEmail(admin.email) } : 'skip',
  )
  const [search, setSearch] = useState('')
  const [addOpen, setAddOpen] = useState(false)
  const [selected, setSelected] = useState<RescuerTableRow | null>(null)
  const [dialogMode, setDialogMode] = useState<DialogMode>('view')
  const [dialogOpen, setDialogOpen] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<RescuerTableRow | null>(null)
  ...
  function openDialog(row: RescuerTableRow, mode: DialogMode) {
    setSelected(row)
    setDialogMode(mode)
    setDialogOpen(true)
  }

  function handleAction(row: RescuerTableRow, action: AdminRowAction) {
    if (action === 'delete') {
      setDeleteTarget(row)
      setDeleteOpen(true)
      return
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      toast.success('Rescuer removed')
      setDeleteOpen(false)
      setDeleteTarget(null)
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Could not remove rescuer')
    } finally {
      setDeleting(false)
    }
  }

  if (!admin || rescuers === undefined) {
    return (
