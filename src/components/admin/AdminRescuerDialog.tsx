...
export function AdminRescuerDialog({
  ...
  const draftSyncKey = open && !isView && rescuerRow ? rescuerRow.email : null
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Could not update rescuer')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isView ? 'View rescuer' : 'Edit rescuer'}</DialogTitle>
