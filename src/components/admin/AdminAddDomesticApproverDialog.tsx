...
export function AdminAddDomesticApproverDialog({ adminEmail, open, onOpenChange }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Could not add approver')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Add Domestic Approver</DialogTitle>
          <DialogDescription>
            Grant domestic portal access to a new email. They will sign in with a one-time code sent to
            that address.
          </DialogDescription>
        </DialogHeader>
  ...
  />
          ...
          <DialogFooter className="pt-2 sm:justify-end">
            ...
            <Button type="submit" disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Add Approver'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
